	...
	invite: function(partyId, userId) {
		...
		if (userId !== party.owner && !_.contains(party.invited, userId)) {
			...
			if (Meteor.isServer && to) {
				// this code only runs on the server. If you didn't want clients to be able to seet it, you move it to a separate file.
				Email.send({ 
	...
	rsvp: function(partyId, rsvp) {
		...
		if (rsvpIndex !== -1) {
			...
			if (Meteor.isServer) {
				...
				// safe on the client since there's only one thread.var modifier = { $set: {} };
				var modifier = { $set: {} };
